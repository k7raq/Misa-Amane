...
Set `DISCORD_TOKEN` to your bot's token wherever you host it (most panels have a "Secrets" or "Environment Variables" section — never hardcode it in `main.py`).
...
Update these constants at the top of `main.py` if anything changes:
- `GUILD_ID` — **set this to your server's ID first.** Right-click your server icon (Developer Mode must be on in Discord settings) → Copy Server ID. This makes commands sync instantly instead of taking up to an hour globally.

"""
Asia Leaderboard Bot — a fresh, simplified rebuild.

Four boards, all Asia region: Top 10, Top 20, Top 30, Top 10 Mobile.
No dodge/protection/cooldown/claim-button system, no review/approval
pipeline — every command acts directly. Simple by design.
...
Storage: simple JSON files in ./data. Swap in a real database later if you
outgrow this.
"""
...
try:
    ...
    pass  # dotenv not installed — fine if the host sets real env vars instead
...
# Only this role can use /createprofile, /deleteprofile, /editprofile, /scan,
# /sendlb, /editspot, /clearspot.
LB_ROLE_ID = int(os.getenv("LB_ROLE_ID", "1543661239652061345"))
# Only this role can use /setannouncement and /scoreannouncement.
...
def next_player_id():
    """Permanent counter — never reuses a deleted profile's ID."""
...
async def on_ready():
    if GUILD_ID:
        ...
        # Clear stale global registrations so nothing shows up twice.
...
async def deleteprofile_cmd(interaction: discord.Interaction, id: str):
    ...
    # Clear (don't renumber) any board spot pointing at the now-deleted profile.
    boards = load_boards()
...
async def editspot_cmd(interaction: discord.Interaction, lb: app_commands.Choice[str], spot: int, id: str, stage: str):
    ...
    if not entry.get("message_id"):
        await interaction.response.send_message(f"That leaderboard hasn't been posted yet — run `/sendlb`.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
...
@createprofile_cmd.error
@editprofile_cmd.error
@deleteprofile_cmd.error
@scan_cmd.error
@sendlb_cmd.error
@editspot_cmd.error
@clearspot_cmd.error
async def lb_role_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CheckFailure):
        await interaction.response.send_message("You don't have the role required to use this.", ephemeral=True)
    else:
        raise error

@setannouncement_cmd.error
@scoreannouncement_cmd.error
async def referee_role_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CheckFailure):
        await interaction.response.send_message("You don't have the role required to use this.", ephemeral=True)
    else:
        raise error
